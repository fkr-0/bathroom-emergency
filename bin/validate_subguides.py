...
if PATH.exists():
    ...
    check(
        data.get("status") == "core-graph-with-six-standalone",
        "released graph status marker missing",
    )
    check(data.get("identity_frozen_on") == "2026-07-26", "identity freeze date missing")
    check(
        set(data.get("standalone_nodes", [])) == {"B", "C", "H", "P", "T", "R"},
        "B/C/H/P/T/R standalone release set drifted",
    )
    check(
...
print(
    ...
    f"prototypes, and B/C/H/P/T/R standalone scope at {VERSION}."
)
